...
**Règles (spec §6 + §12) :**
- Coût ingrédient **article** = `quantite × prixConverti`, où `prixConverti = prixUnitaireUSD × facteur(article.unite → unite consommée)` — si `prixUnitaireUSD` null → ingrédient **sans prix** (ajouté à `ingredientsSansPrix`, non compté) ; si `facteur` null (unité-emballage : diviser par `poidsEmballage`; sinon incompatible) → sans prix.
