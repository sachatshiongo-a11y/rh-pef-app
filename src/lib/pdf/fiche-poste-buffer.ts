...
export async function genererFichePostePdf(ficheId: string): Promise<{ buffer: Buffer; nomFichier: string } | null> {
  ...
  // Classe déduite : catégorie professionnelle la plus fréquente parmi les salariés du poste.
  ...
  const dominante = [...compte.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? null;
  const classe = labelCategoriePro(dominante);
