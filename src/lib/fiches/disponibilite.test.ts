...
/** Jour de référence des tests ; par défaut, le stock a bougé la veille (à jour). */
const AUJ = "2026-09-24";
const depot = (q: string, dernierMouvement: string | null = "2026-09-23"): StockArticle => ({ depot: q, restaurant: null, dernierMouvement });
const dispo = (f: FicheDispo, c: ContexteDispo, aujourdhui = AUJ) => calculerDisponibilite(f, c, aujourdhui);
...
describe("calculerDisponibilite — portions et ingrédient limitant", () => {
  it("l'ingrédient limitant est celui qui donne le moins de portions", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE, OEUF], stocks: { farine: depot("10"), oeuf: depot("9") } }));
  ...
  it("égalité : le limitant est le premier dans l'ordre de la fiche", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE, OEUF], stocks: { farine: depot("5"), oeuf: depot("10") } }));
  ...
  it("arrondi à l'entier inférieur, une seule fois, à la fin", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE], stocks: { farine: depot("1") } }));
  ...
  it("convertit g → kg et ml → l", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE, CREME], stocks: { farine: depot("2"), creme: depot("1") } }));
  ...
  it("une unité-emballage (« 500 GR ») se convertit comme pour le coût : par le poids du paquet", () => {
    ...
    const r = dispo(f, ctx({ articles: [PATES], stocks: { pates: depot("3") } }));
  ...
  it("une quantité à 0 vaut « non renseignée » (règle du coût) : À vérifier, jamais un faux Disponible", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE, OEUF], stocks: { farine: depot("10"), oeuf: depot("3") } }));
  ...
  it("une quantité négative est une saisie invalide : À vérifier", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE], stocks: { farine: depot("10") } }));
    expect(r.etat).toBe("A_VERIFIER");
...
describe("calculerDisponibilite — inconnus annoncés, jamais comptés zéro", () => {
  ...
  it("pas de stock enregistré (ni dépôt ni restaurant) → À vérifier", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" })] });
    const r = dispo(f, ctx({ articles: [FARINE] }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("stock total négatif → À vérifier", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" })] });
    const r = dispo(f, ctx({ articles: [FARINE], stocks: { farine: depot("-2") } }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("dépôt + restaurant sont additionnés", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" })] });
    const stocks = { farine: { dernierMouvement: "2026-09-23", depot: "2", restaurant: { etat: "OK" as const, quantite: "1.5", dateComptage: "2026-09-22" } } };
    const r = dispo(f, ctx({ articles: [FARINE], stocks }));
  ...
  it("restaurant seul (aucune ligne Stock) : le comptage suffit", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" })] });
    const stocks = { farine: { dernierMouvement: "2026-09-23", depot: null, restaurant: { etat: "OK" as const, quantite: "2", dateComptage: "2026-09-22" } } };
    expect(dispo(f, ctx({ articles: [FARINE], stocks })).portions).toBe(2);
  });

  it("restaurant dans une unité non convertible → À vérifier (restaurant)", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" })] });
    const stocks = { farine: { dernierMouvement: "2026-09-23", depot: "5", restaurant: { etat: "UNITE_NON_CONVERTIBLE" as const, articleResto: "Farine (sac)" } } };
    const r = dispo(f, ctx({ articles: [FARINE], stocks }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("RUPTURE contre À VÉRIFIER : l'inconnu l'emporte", () => {
    ...
    const r = dispo(f, ctx({ articles: [FARINE, OEUF], stocks: { farine: depot("0") } }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("RUPTURE : les ingrédients à 0 portion sont nommés", () => {
    const f = fiche({ id: "p", ingredients: [ing("Farine", "kg", "1", { articleId: "farine" }), ing("Œuf", "pièce", "1", { articleId: "oeuf" })] });
    const r = dispo(f, ctx({ articles: [FARINE, OEUF], stocks: { farine: depot("0.5"), oeuf: depot("4") } }));
  ...
  it("une fiche sans ingrédient est À vérifier (« aucun ingrédient »)", () => {
    const r = dispo(fiche({ id: "p", ingredients: [] }), ctx({}));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("des portions inexploitables rendent le plat À vérifier", () => {
    ...
    expect(dispo(f, ctx({ articles: [FARINE], stocks: { farine: depot("3") } })).raisons)
      .toEqual([{ motif: "PORTIONS_INVALIDES", ingredient: null }]);
  });
});

describe("calculerDisponibilite — stock figé (décision Direction 2026-09-24)", () => {
  // Référence : 24/09/2026. Seuil : plus de JOURS_MAX_SANS_MOUVEMENT (7) jours sans mouvement.
  const SAUMON = art("saumon", "Saumon fumé", "kg");
  const platSaumon = fiche({ id: "p", ingredients: [ing("Saumon fumé", "g", "40", { articleId: "saumon" })] });
  const avec = (stock: StockArticle) => dispo(platSaumon, ctx({ articles: [SAUMON], stocks: { saumon: stock } }));

  it("le seuil est une constante nommée : 7 jours", () => {
    expect(JOURS_MAX_SANS_MOUVEMENT).toBe(7);
  });

  it("mis à jour il y a 3 jours : pris en compte", () => {
    const r = avec(depot("4.44", "2026-09-21"));
    expect(r.etat).toBe("DISPONIBLE");
    expect(r.portions).toBe(111);
    expect(r.articles[0]).toMatchObject({ dernierMouvement: "2026-09-21", motif: null, portions: 111 });
  });

  it("le cas réel : 4,44 kg inchangés depuis le 10/07 → À vérifier, plus « 111 portions »", () => {
    const r = avec(depot("4.44", "2026-07-10"));
    expect(r.etat).toBe("A_VERIFIER");
    expect(r.portions).toBeNull();
    expect(r.raisons).toEqual([{ motif: "STOCK_NON_MIS_A_JOUR", ingredient: "Saumon fumé", depuis: "2026-07-10" }]);
    expect(libelleRaison(r.raisons[0]!)).toBe("Saumon fumé : stock non mis à jour depuis le 10/07");
    expect(r.articles[0]).toMatchObject({ dernierMouvement: "2026-07-10", motif: "STOCK_NON_MIS_A_JOUR", depuis: "2026-07-10", portions: null });
  });

  it("mis à jour il y a 10 jours : À vérifier", () => {
    const r = avec(depot("4.44", "2026-09-14"));
    expect(r.etat).toBe("A_VERIFIER");
    expect(libelleRaison(r.raisons[0]!)).toBe("Saumon fumé : stock non mis à jour depuis le 14/09");
  });

  it("jamais mis à jour (aucun mouvement) : À vérifier, sans date", () => {
    const r = avec(depot("4.44", null));
    expect(r.etat).toBe("A_VERIFIER");
    expect(r.raisons).toEqual([{ motif: "STOCK_JAMAIS_MIS_A_JOUR", ingredient: "Saumon fumé" }]);
    expect(libelleRaison(r.raisons[0]!)).toBe("Saumon fumé : stock jamais mis à jour");
    expect(r.articles[0]!.dernierMouvement).toBeNull();
  });

  it("limite exacte : 7 jours pile compte encore, 8 jours non", () => {
    expect(joursEntre("2026-09-17", AUJ)).toBe(7);
    expect(avec(depot("4.44", "2026-09-17")).etat).toBe("DISPONIBLE");
    expect(avec(depot("4.44", "2026-09-16")).etat).toBe("A_VERIFIER");
    // À travers une fin de mois : 31/08 → 07/09 = 7 jours.
    expect(joursEntre("2026-08-31", "2026-09-07")).toBe(7);
    expect(avec(depot("4.44", "2026-08-31")).etat).toBe("A_VERIFIER");
    expect(dispo(platSaumon, ctx({ articles: [SAUMON], stocks: { saumon: depot("4.44", "2026-08-31") } }), "2026-09-07").etat).toBe("DISPONIBLE");
  });

  it("la date de référence est un paramètre : le même stock, vu 5 jours plus tard, devient à vérifier", () => {
    const c = ctx({ articles: [SAUMON], stocks: { saumon: depot("4.44", "2026-09-20") } });
    expect(dispo(platSaumon, c, "2026-09-24").etat).toBe("DISPONIBLE");
    expect(dispo(platSaumon, c, "2026-09-29").etat).toBe("A_VERIFIER");
  });

  describe("restaurant : même règle quand un comptage entre dans le calcul", () => {
    const resto = (dateComptage: string) => ({ etat: "OK" as const, quantite: "1", dateComptage });

    it("comptage d'il y a 3 jours, dépôt à jour : pris en compte", () => {
      const r = avec({ depot: "3.44", dernierMouvement: "2026-09-23", restaurant: resto("2026-09-21") });
      expect(r.etat).toBe("DISPONIBLE");
      expect(r.portions).toBe(111);
    });

    it("comptage d'il y a 10 jours : À vérifier, même si le dépôt est à jour", () => {
    ...
    it("limite exacte au restaurant : 7 jours pile compte encore, 8 jours non", () => {
      expect(avec({ depot: null, dernierMouvement: null, restaurant: resto("2026-09-17") }).etat).toBe("DISPONIBLE");
      expect(avec({ depot: null, dernierMouvement: null, restaurant: resto("2026-09-16") }).etat).toBe("A_VERIFIER");
    });

    it("restaurant seul (aucune ligne Stock au dépôt) : le dépôt n'entre pas dans le calcul, sa date non plus", () => {
      const r = avec({ depot: null, dernierMouvement: null, restaurant: resto("2026-09-22") });
      expect(r.etat).toBe("DISPONIBLE");
    });
  });
});
...
describe("calculerDisponibilite — sous-recettes", () => {
  ...
  it("une sous-recette avec rendement est éclatée jusqu'aux articles de base", () => {
    ...
    const r = dispo(plat, ctx({ fiches: [sauce, plat], articles: [FARINE, CREME], stocks: { creme: depot("1"), farine: depot("1") } }));
  ...
  it("division exacte : 200 g d'une sauce de 300 g (0,5 l de crème), 1 l en stock → 3 portions, pas 2", () => {
    ...
    const r = dispo(plat, ctx({ fiches: [s, plat], articles: [CREME], stocks: { creme: depot("1") } }));
    expect(r.portions).toBe(3);
  });
  ...
  it("une sous-recette sans rendement → À vérifier", () => {
    ...
    const r = dispo(plat, ctx({ fiches: [s, plat], articles: [FARINE, CREME], stocks: { creme: depot("5"), farine: depot("5") } }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("une sous-recette consommée à 0 dans le plat → À vérifier", () => {
    ...
    const r = dispo(plat, ctx({ fiches: [sauce, plat], articles: [FARINE, CREME], stocks: { creme: depot("5"), farine: depot("5") } }));
  ...
  it("une boucle est détectée, sans récursion infinie", () => {
    ...
    const r = dispo(a, ctx({ fiches: [a, b] }));
    expect(r.etat).toBe("A_VERIFIER");
  ...
  it("une sous-recette se compte en rendements (fournées entières), pas en portions", () => {
    const r = dispo({ ...sauce, nbPortions: 4 }, ctx({ fiches: [sauce], articles: [FARINE, CREME], stocks: { creme: depot("3"), farine: depot("1") } }));
  ...
  it("le même article en direct et dans une sous-recette : les besoins s'additionnent", () => {
    ...
    const r = dispo(plat, ctx({ fiches: [sauce, plat], articles: [FARINE, CREME], stocks: { creme: depot("2"), farine: depot("1") } }));
