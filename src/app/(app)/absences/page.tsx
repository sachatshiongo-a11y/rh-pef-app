...
export default async function AbsencesPage({
  ...
  searchParams: Promise<{ mois?: string; annee?: string; type?: string; emp?: string; vue?: string; debut?: string }>;
}) {
  await verifySession();
  ...
  if (vue === "semaine") {
    ...
    navPrec = `vue=semaine&debut=${isoJour(new Date(lundi.getTime() - 7 * 86_400_000))}`;
    navSuiv = `vue=semaine&debut=${isoJour(new Date(lundi.getTime() + 7 * 86_400_000))}`;
    navAuj = `vue=semaine`;
  } else {
    ...
    navPrec = `vue=mois&mois=${mp.m}&annee=${mp.a}`;
    navSuiv = `vue=mois&mois=${ms.m}&annee=${ms.a}`;
    navAuj = `vue=mois`;
  }
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          <Link href={`/absences?${navPrec}${filtresQS}`} className="rounded-md border px-3 py-1.5 hover:bg-accent">← Préc.</Link>
          <Link href={`/absences?${navAuj}${filtresQS}`} className="rounded-md border px-3 py-1.5 hover:bg-accent">Aujourd&apos;hui</Link>
          <Link href={`/absences?${navSuiv}${filtresQS}`} className="rounded-md border px-3 py-1.5 hover:bg-accent">Suiv. →</Link>
        </div>
      </div>
      ...
      <form method="get" className="mb-4 flex flex-wrap items-end gap-3">
        ...
        {(filtreType || filtreEmp) && (
          <Link href={`/absences?${navAuj}`} className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent">
            Réinitialiser
          </Link>
        )}
      </form>
