...
export type SPHistorique = { annee?: string; mois?: string; statut?: string };

export async function HistoriquePaie({ sp }: { sp: SPHistorique }) {
  ...
  return (
    <div>
      <form method="GET" action="/paie" className="mb-5 flex flex-wrap items-end gap-2 rounded-xl border bg-card p-2.5 sm:gap-3 sm:p-3">
        <input type="hidden" name="vue" value="historique" />
        ...
        {(annee || mois || statut) && (
          <Link href="/paie?vue=historique" className="rounded-md border px-4 py-1.5 text-sm font-medium hover:bg-accent">Réinitialiser</Link>
        )}
      </form>
